...
- Seeding `entities`/`edges` directly via mongosh requires the full `prov` object (`harness/agent/method/confidence/extracted_at`) or schema validation rejects the insert.
- The `mongo:8` docker tag is now 8.1+ (native `$rankFusion`); to test the in-process rank-fusion path on an older server, spin up `docker run -d --name me0-mongo80 -p 127.0.0.1:27018:27017 mongo:8.0`.
- MCP stdio testing needs a held-open FIFO — closing stdin right after writing (e.g. a heredoc) yields "Operation interrupted because client was closed".
- `episode_start` rejects `harness: "cli"` with an opaque "Document failed validation" — the episodes collection's Mongo validator only allows `claude-code|codex|pi|hermes|openclaw|devin|other`; use `"other"` for ad-hoc CLI tests.
- A pre-existing `me0-mongo` container can carry stale collection validators from older branches and fail all writes with "Document failed validation" — reset with `docker exec me0-mongo mongosh --quiet --eval 'db.getSiblingDB("me0").dropDatabase()'` then re-run `me0 init`.
- LLM extraction (PR #17+): fake an OpenAI-compatible endpoint with a tiny `Bun.serve` script answering POST `/chat/completions` with `{"choices":[{"message":{"content":"[...json array...]"}}]}`; point `ME0_LLM_BASE_URL=http://127.0.0.1:<port>/v1` + `ME0_LLM_MODEL=fake`. Dedupe is global on normalized text, so reuse of the same fake items yields `+0 memories (N duplicates)` — vary the fake texts when you need to prove new inserts. Don't `pkill -f <script name>` from the same exec call (it matches the wrapping shell and kills itself); kill by port (`fuser <port>/tcp`) instead.
